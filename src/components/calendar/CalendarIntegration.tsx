...
import { Calendar as CalendarIcon, Check, Loader, AlertCircle } from 'lucide-react';
...
export const MOCK_CALENDAR_EVENTS = [
  ...
  },
  {
    ...
    end: new Date(new Date().setDate(new Date().getDate() + 1)).setHours(11, 30, 0, 0),
    location: 'Google Meet',
    attendees: 2,
  ...
  },
  {
    ...
    attendees: 35,
    description: 'Monthly engineering department meeting to discuss company updates and technical roadmap.'
  }
];

export default function CalendarIntegration({ onConnect, isConnected }: CalendarIntegrationProps) {
  ...
  const handleConnect = async (provider: 'google' | 'microsoft') => {
    ...
    try {
      ...
      onConnect(provider);
      toast.success(`Connected to ${provider === 'google' ? 'Google' : 'Microsoft'} Calendar`);
    } catch (error) {
      console.error(`Error connecting to ${provider} calendar:`, error);
      setConnectionError(`Failed to connect to ${provider} Calendar. ${error instanceof Error ? error.message : 'Please check your permissions and try again.'}`);
    ...
    <Card className="p-4 border-l-4 border-l-primary">
            ...
            <>
              ...
              >
                {isConnecting === 'microsoft' ? (
                  ...
                  <svg width="16" height="16" viewBox="0 0 21 21" className="mr-2">
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <ul className="text-xs text-gray-600 dark:text-gray-400 space-y-1 list-disc pl-5">
          <li>Calendar integration shows your meetings alongside your work schedule</li>
          <li>We only access your meeting times, titles, and attendee counts</li>
