  ...
  return (
    <div>
      ...
      <Alert
        ...
        className="mb-6"
      >
        <p className="text-sm">Your preferences help us optimize team coordination and improve the hybrid work experience.</p>
      </Alert>
      
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="preferredOfficeDays" className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div>
          ...
          <div className="space-y-2">
            {[
              { id: 'collaboration', label: 'In-person collaboration with teammates' },
              { id: 'focus', label: 'Better focus environment than home' },
              { id: 'separation', label: 'Separation between work and personal life' },
              ...
              <div key={option.id} className="flex items-start">
                ...
                />
                <label htmlFor={`motivator-${option.id}`} className="ml-2 block text-sm text-gray-700">
                  {option.label}
                </label>
              </div>
            ))}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            What are your main barriers to office attendance? (Select all that apply)
          </label>
          <div className="space-y-2">
            {[
              { id: 'commute', label: 'Long commute time' },
              { id: 'productivity', label: 'Lower productivity than at home' },
              { id: 'flexibility', label: 'Need flexibility for personal commitments' },
              { id: 'environment', label: 'Office environment (noise, distractions)' },
              ...
              <div key={option.id} className="flex items-start">
                <input
                  ...
                  className="h-4 w-4 mt-0.5 border-gray-300 rounded text-primary focus:ring-primary"
                />
                <label htmlFor={`barrier-${option.id}`} className="ml-2 block text-sm text-gray-700">
                  {option.label}
                </label>
              </div>
            ))}
        ...
        <div>
          <label htmlFor="additionalFeedback" className="block text-sm font-medium text-gray-700 mb-2">
            Any additional thoughts about your hybrid work preferences? (Optional)
          </label>
          <textarea
