...
export default function RewardCard({ reward, onRedeem, disabled = false, className = '' }: RewardCardProps) {
  const handleRedeem = () => {
    if (!disabled) {
      onRedeem(reward.id);
    }
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-2">
          <h3 className="text-sm sm:text-lg font-semibold text-gray-900 dark:text-white line-clamp-1">{reward.title}</h3>
        ...
        <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 mb-3 line-clamp-2">{reward.description}</p>
