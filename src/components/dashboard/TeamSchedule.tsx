...
import { Users, Calendar, ChevronRight, Building2, Home, Clock } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const generateTeamData = () => {
      return Array.from({ length: 4 }, (_, i) => ({
        ...
        role: ['Senior Software Engineer', 'Product Manager', 'UX Designer', 'Engineering Manager'][i],
        avatar: [
        ...
        schedule: currentWeekDays.reduce((acc, day) => {
        ...
        votedDays: i % 2 === 0 
          ? [format(addDays(currentWeekStart, 1), 'yyyy-MM-dd'), format(addDays(currentWeekStart, 3), 'yyyy-MM-dd')]
          : [format(addDays(currentWeekStart, 0), 'yyyy-MM-dd'), format(addDays(currentWeekStart, 2), 'yyyy-MM-dd')]
      }));
    };

    const teamData = getConsistentMockData('teamScheduleMembers', generateTeamData);
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base sm:text-lg font-medium text-gray-900 dark:text-white">My Team's Schedule</h3>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Who's in the office this week</p>
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
          <Calendar className="h-4 w-4" />
          <span className="hidden sm:inline">Week of</span> {format(currentWeekStart, 'MMM d, yyyy')}
        </div>
      </div>
              ...
              <div className="flex-1 min-w-0">
                  ...
                  {member.votedDays && member.votedDays.length > 0 && (
                    <div className="flex items-center gap-2 text-xs animate-fadeIn">
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 text-primary font-medium">
                        ...
                        <span>Next Week:</span>
                        <span className="hidden sm:inline">{member.votedDays.map(date => format(new Date(date), 'EEE')).join(', ')}</span>
                        <span className="sm:hidden">{member.votedDays.length} day{member.votedDays.length !== 1 ? 's' : ''}</span>
                      </div>
                    </div>
                  )}
                </div>
                <div className="mt-3 grid grid-cols-5 gap-1 sm:gap-2">
                  {currentWeekDays.map((day) => {
                    ...
                    const isAnchorDay = day.getDay() === 2 || day.getDay() === 4;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {isAnchorDay && (
                            <span className="absolute -top-2 -right-2 w-4 h-4 bg-primary text-white text-[8px] flex items-center justify-center rounded-full">A</span>
                          )}
                        </div>
                      </div>
                    );
