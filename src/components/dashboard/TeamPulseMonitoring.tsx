...
export default function TeamPulseMonitoring() {
  ...
  const [filterBy, setFilterBy] = useState<'all' | 'challenging' | 'neutral' | 'good'>('all');
  ...
  const filteredMembers = teamPulseData.filter(member => 
    filterBy === 'all' || member.currentMood === filterBy
  );
    ...
    <Card className="p-4 sm:p-6 overflow-hidden">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base sm:text-lg font-medium text-gray-900 dark:text-white">Team Pulse Monitor</h3>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Keep track of your team's wellbeing</p>
          </div>
        </div>
          ...
          {sortedMembers.map((member) => (
            <motion.div
              ...
              className={`
                ...
                ${member.requiresAttention ? 'bg-error/5 dark:bg-error/10 border-error/20' : 
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex justify-between items-center">
          <div className="text-xs text-gray-500 dark:text-gray-400">
            {sortedMembers.filter(m => m.requiresAttention).length} team members need attention
          </div>
