...
export default function TeamComplianceReport() {
  ...
  const meetingRequirements = complianceData.filter(item => item.compliance_rate >= 1).length;
    ...
    const generateComplianceData = () => {
      ...
      return teamMembers.map(member => {
        ...
        return {
          ...
          user_name: member.member_name || member.user?.full_name || 'Team Member',
    ...
    <Card className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            leftIcon={showChart ? <Calendar className="h-4 w-4" /> : <BarChart3 className="h-4 w-4" />}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-card-hover p-4 rounded-lg">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Overall Compliance</p>
        ...
        <div className="bg-card-hover p-4 rounded-lg">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Meeting Requirements</p>
          <div className="flex items-end gap-2">
            <span className="text-2xl font-semibold text-gray-900 dark:text-white">
        ...
        <div className="bg-card-hover p-4 rounded-lg">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Required Office Days</p>
          <div className="flex items-end gap-2">
            <span className="text-2xl font-semibold text-gray-900 dark:text-white">
            ...
            <div className="h-80 mt-6">
              <div className="flex h-full items-end gap-2">
                {complianceData.map(item => (
                  <div key={item.user_id} className="flex-1 flex flex-col items-center gap-2">
                    <div 
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {complianceData.map((item) => (
                    <tr key={item.user_id}>
                      ...
                      <td className="px-4 py-3">
                        <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ width: `${Math.min(item.compliance_rate * 100, 100)}%` }}
                          ></div>
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(item.compliance_rate)}`}>
                          {item.compliance_rate >= 1 && <Check className="h-3 w-3" />}
