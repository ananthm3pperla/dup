...
describe('PasswordStrengthIndicator', () => {
  ...
  it('shows correct requirement statuses', () => {
    ...
    const uppercaseRequirement = screen.getByText('At least one uppercase letter');
    ...
    const specialCharRequirement = screen.getByText('At least one special character');
    expect(specialCharRequirement.parentElement).not.toHaveClass('text-green-600');
  });

  it('applies custom class name', () => {
    const { container } = render(<PasswordStrengthIndicator password="abc" className="custom-class" />);
