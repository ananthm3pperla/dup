...
export default function AuthCallback() {
  ...
  const [errorType, setErrorType] = useState<
    "auth" | "permission" | "other" | null
  >(null);
  const [success, setSuccess] = useState(false);
  const [processingStage, setProcessingStage] = useState<
    "verifying" | "setting_up" | "redirecting"
  >("verifying");
  ...
  useEffect(() => {
    ...
    if (urlError || hashError) {
      const errorMessage =
        urlErrorDescription || hashErrorDescription || "Authentication failed";
      ...
      if (user && !authLoading && !teamLoading) {
        try {
          ...
          if (pendingInviteCode) {
            ...
            navigate("/join-team", {
              state: { inviteCode: pendingInviteCode },
            });
            return;
          }
          ...
          setError(
            "Error setting up your account. Please try refreshing the page or contact support.",
          );
          setErrorType("other");
        }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
          ...
          <Alert
            variant="error"
            title={
              errorType === "auth" ? "Authentication Failed" : "Setup Error"
            }
          >
          ...
          {errorType === "permission" && (
            <div className="mt-6 bg-warning/10 p-4 rounded-lg">
              <p className="text-sm text-warning mb-2">
                It looks like we're having some database permission issues. You
                can:
              </p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <LoadingState message="Completing sign in..." variant="pulse" />
    </div>
  );
}
