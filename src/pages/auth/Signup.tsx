...
export default function Signup() {
  ...
  const { user, signUp, signInWithGoogle, signInWithMicrosoft, error: authError, enterDemoMode } = useAuth();
  ...
  const handleSubmit = async (name: string, email: string, password: string) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to create account. Please try again later.';
      ...
      if (errorMessage.toLowerCase().includes('database') || 
        ...
        toast.error("Registration service is currently experiencing issues. Try demo mode to explore the app.");
      }
    } finally {
        ...
        <div className="bg-white dark:bg-gray-800 px-4 py-8 shadow sm:rounded-lg sm:px-10">
          {showDemoOption && (
            <Alert 
              ...
              className="mb-6"
            >
              <p className="mb-2">Our database is currently experiencing issues. You can:</p>
