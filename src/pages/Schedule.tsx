...
export default function Schedule() {
  ...
  const { currentWeek, userSchedule, loadUserSchedule, loadTeamSchedule } =
    useSchedule();

  // UI states
  const [isLoading, setIsLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<"current" | "next" | "team">(
    "current",
  );
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        await Promise.all([loadUserSchedule(), loadTeamSchedule()]);
      } catch (err) {
        ...
        <div className="mx-0 my-4 p-3 rounded-md bg-primary/10 dark:bg-primary/20 border border-primary/20 dark:border-primary/30">
          <div className="flex items-center gap-2 text-primary-700 dark:text-primary-300">
            ...
            <p className="text-sm">
              ...
              {currentTeam.rto_policy.required_days} days in the office per week
              {currentTeam.rto_policy.core_hours && (
                <span>
                  {" "}
                  with core hours from {
                    currentTeam.rto_policy.core_hours.start
                  }{" "}
                  to {currentTeam.rto_policy.core_hours.end}
                </span>
              )}
            </p>
          </div>
          ...
          <div>
            <div className="mb-4 flex items-center">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                Days when most team members are in the office
              </span>
            </div>
            <WeeklyCalendar
              viewMode="current"
        ...
        {activeTab === "team" && isTeamLeader && (
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              View your team's schedule and coordinate in-office days for better
              collaboration.
            </p>
      ...
      <Card className="p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-gray-800 border border-primary/30 rounded-lg p-4 shadow-md">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {currentTeam?.rto_policy.required_days || 3} days per week in
                  office
                </p>
              </div>
            </div>
            {currentTeam?.rto_policy.core_hours && (
              <div className="text-sm text-gray-600 dark:text-gray-300 flex items-center gap-2">
                ...
                Core hours: {currentTeam.rto_policy.core_hours.start} -{" "}
                {currentTeam.rto_policy.core_hours.end}
              </div>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg p-4 shadow-md">
            ...
            {userSchedule.filter((s) => s.work_type === "office").length >=
            (currentTeam?.rto_policy.required_days || 3) ? (
              ...
              <div className="text-sm text-warning flex items-center gap-2">
                ...
                Need{" "}
                {(currentTeam?.rto_policy.required_days || 3) -
                  userSchedule.filter((s) => s.work_type === "office")
                    .length}{" "}
                more office days
              </div>
            )}
