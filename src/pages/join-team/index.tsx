...
import { Building2, ChevronLeft, AlertCircle, DoorOpen, Beaker } from 'lucide-react';
...
export default function JoinTeam() {
  ...
  const [teamInfo, setTeamInfo] = useState<{ id: string; name: string; description?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (location.state?.inviteCode) {
      setFormData(prev => ({ ...prev, inviteCode: location.state.inviteCode }));
    }
  ...
  const validateInviteCode = async () => {
    ...
    try {
      ...
      setErrors({ inviteCode: 'Error validating invite code. Please try again.' });
      return false;
    } finally {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ 
        inviteCode: err instanceof Error 
          ? err.message 
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-bold text-gray-900 dark:text-white">
          {user ? "Join a Team" : (step === 1 ? "Join a Team" : "Create Your Account")}
        </h2>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between">
                <div className={`flex items-center ${step >= 1 ? 'text-primary' : 'text-gray-400'}`}>
                  <div className={`flex items-center justify-center h-8 w-8 rounded-full ${step >= 1 ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
                    1
                  </div>
                  <span className="ml-2 text-sm font-medium">Invite Code</span>
                </div>
                <div className={`flex-1 mx-4 h-0.5 ${step > 1 ? 'bg-primary' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                <div className={`flex items-center ${step >= 2 ? 'text-primary' : 'text-gray-400'}`}>
                  <div className={`flex items-center justify-center h-8 w-8 rounded-full ${step >= 2 ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
                    2
                  </div>
          ...
          {!user && step === 2 && (
            <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
              <div className="text-center text-sm">
                ...
                <Link to="/login" className="font-medium text-primary hover:text-primary-light dark:hover:text-primary-light">
                  Sign in
                </Link>
              </div>
            </div>
