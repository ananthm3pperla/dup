...
export function generateTeamMembers(count: number = 5): TeamMember[] {
  ...
  const generateMembersData = () => {
    ...
    const additionalMembers = Array.from(
      { length: Math.max(0, count - fixedMembers.length) },
      (_, i) => ({
        id: faker.string.uuid(),
        member_id: faker.string.uuid(),
        ...
        member_workLocation:
          i % 3 === 0 ? "Remote" : i % 3 === 1 ? "Hybrid" : "Office",
        member_attendance: {
          total: 100 + i,
          streak: 5 + (i % 10),
          lastVisit: new Date().toISOString(),
...
export function getDepartmentSummary(departmentName: string) {
  const generateSummaryData = () => {
    ...
    return {
      ...
      remoteCount:
        headCount - Math.floor(headCount * (inOfficePercentage / 100)),
      department_members: [...fixedMembers, ...additionalMembers].filter(
        (member) =>
          member.member_department === departmentName ||
          departmentName === "All",
      ),
    };
  };
  ...
  return getConsistentMockData(
    `department_summary_${departmentName}`,
    generateSummaryData,
  );
}
...
export function generateMockRewards(count: number = 10): Reward[] {
  const generateRewardsData = () => {
    ...
    const fixedRewards = [
      {
        ...
        description:
          "Redeem for an additional day of remote work beyond your regular allocation.",
        imageUrl:
          "https://images.unsplash.com/photo-1522881193457-37ae97c905bf?w=500&h=300&q=80",
        pointsCost: 200,
      ...
      },
      {
        ...
        description:
          "Schedule a 30-minute one-on-one coffee chat with our CEO to share your ideas.",
        imageUrl:
          "https://images.unsplash.com/photo-1559925393-8be0ec4767c8?w=500&h=300&q=80",
        pointsCost: 350,
      ...
      },
      {
        ...
        title: "Team Lunch",
        description:
          "Treat your team to lunch on the company (up to $25 per person).",
        imageUrl:
          "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=500&h=300&q=80",
        pointsCost: 300,
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1577563908411-5077b6dc7624?w=500&h=300&q=80",
        pointsCost: 500,
        category: "Gift Cards",
        availability: "limited" as const,
      ...
      },
      {
        ...
        imageUrl:
          "https://images.unsplash.com/photo-1546435770-a3e426bf472b?w=500&h=300&q=80",
        pointsCost: 1000,
        category: "Technology",
        availability: "limited" as const,
        expiresAt: addDays(new Date(), 60).toISOString(),
    ...
    const additionalRewards = Array.from(
      { length: Math.max(0, count - fixedRewards.length) },
      (_, i) => ({
        ...
        availability:
          i % 3 === 0 ? ("unlimited" as const) : ("limited" as const),
