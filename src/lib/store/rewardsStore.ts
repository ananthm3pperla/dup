...
export const useRewardsStore = create<RewardsStore>((set, get) => ({
    ...
    try {
      if (isDemoMode()) {
        const mockBalance = getConsistentMockData('rewardBalance', {
          ...
          total_used: 9,
          streak: 2,
          last_office_day: new Date(Date.now() - 86400000).toISOString().split('T')[0],
  ...
  requestRemoteDays: async (userId: string, teamId: string, date: string, days: number, reason?: string) => {
    ...
    try {
      ...
      const existingRequests = await database.get(`remote_requests:${userId}`) || [];
  ...
  cancelRequest: async (requestId: string) => {
    ...
    try {
      ...
      const state = get();
      const userId = state.requests.find(r => r.id === requestId)?.id.split('_')[0]; // Extract user ID

      if (userId) {
        const requests = await database.get(`remote_requests:${userId}`) || [];
